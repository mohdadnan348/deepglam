...
// VERIFY EMPLOYEE CODE (replace existing handler with this)
  ...
  try {
    ...
    try {
      ...
      if (user) {
        ...
        return res.json({ ok: true, data: { employeeName: name, staffUserId: user._id, role: user.role || "staff" } });
      }
    } catch (uErr) {
      console.warn("verifyEmployeeCode - user lookup error:", uErr?.message || uErr);
    }
    ...
    try {
      ...
      if (staff) {
        ...
        // staff.userId?._id may be undefined for older records; fallback to staff._id
        const staffUserId = staff.userId?._id || staff._id;
        return res.json({
          ok: true,
          data: { employeeName: name, staffUserId, role: "staff" },
        });
      }
    } catch (sErr) {
      console.warn("verifyEmployeeCode - staff lookup error:", sErr?.message || sErr);
    }

    // 3) Try Seller collection (if sellers can also have employeeCode)
    try {
      ...
      if (seller) {
        ...
        const sellerUserId = seller.userId?._id || seller._id;
        return res.json({
          ok: true,
          data: { employeeName: name, staffUserId: sellerUserId, role: "seller" },
        });
      }
    } catch (selErr) {
      console.warn("verifyEmployeeCode - seller lookup error:", selErr?.message || selErr);
    }
