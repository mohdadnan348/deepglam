...
const { uploadRawBufferToCloudinary, saveBufferLocally } = require('../utils/upload.helper'); 
...
/* -------------------- CREATE PRODUCT (Cloudinary + Local) -------------------- */
exports.createProduct = async (req, res) => {
  try {
    ...
    if (!productName || !mainCategory || !subCategory || !purchasePrice || !productType || !mainImage) {
    ...
    // Upload mainImage to Cloudinary if it's a local file (uri/base64)
    let mainImgObj = toImageObj(mainImage);
    if (mainImage && !mainImage.url) {
      try {
        const result = await uploadRawBufferToCloudinary(Buffer.from(mainImage.base64 || '', 'base64'), {
          folder: 'products',
          publicId: `product_${Date.now()}`
        });
        mainImgObj = { url: result.secure_url, public_id: result.public_id };
      } catch (err) {
        console.warn('Cloudinary upload failed, saving locally:', err.message);
        const local = saveBufferLocally(Buffer.from(mainImage.base64 || '', 'base64'), `product_${Date.now()}`, 'products');
        mainImgObj = { url: local.url, public_id: '' };
      }
    }
    ...
    for (const img of images) {
      let imgObj = toImageObj(img);
      if (img && !img.url && img.base64) {
        try {
          const result = await uploadRawBufferToCloudinary(Buffer.from(img.base64, 'base64'), { folder: 'products' });
          imgObj = { url: result.secure_url, public_id: result.public_id };
        } catch (err) {
          const local = saveBufferLocally(Buffer.from(img.base64, 'base64'), `product_${Date.now()}`, 'products');
          imgObj = { url: local.url, public_id: '' };
        }
      }
      if (imgObj) imagesArr.push(imgObj);
    }
