...
const invoiceItemSchema = new mongoose.Schema({
  ...
  unitPricePaise: { type: Number, required: true, min: 0 },   // integer (paise)
  discountPaise: { type: Number, default: 0, min: 0 },
  gstPercentage: { type: Number, default: 0, min: 0 },
  gstPaise: { type: Number, default: 0, min: 0 },
  lineTotalPaise: { type: Number, required: true, min: 0 },   // (= taxable+gst)
}, { _id: false });
...
const invoiceSchema = new mongoose.Schema({
  ...
  number:   { type: String, unique: true, index: true }, // e.g. INV-2025-08-000123
  ...
  // Gateway-specific (for Paytm Dynamic QR etc.)
  paytm: {
    qrCodeId: String,
    qrData: String,
    qrImageB64: String,
  ...
  phonepe: {
    merchantTransactionId: String, // you generate at init
    merchantOrderId: String,       // optional: map to invoice.number
  },

}, { timestamps: true, versionKey: true }); // versionKey for optimistic locking

// Helper to apply a successful/captured payment atomically on the doc
